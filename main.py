...
# NEW: Stop Loss and Take Profit settings
stop_loss = 0  # Stop bot if cumulative profit drops below this (negative number)
take_profit = 0  # Stop bot if cumulative profit reaches this (positive number)
stop_reason = ""  # Reason for stopping (stop_loss or take_profit)
...
async def auto_trader():
    ...
    while auto_trader_running:
        ...
        # Check stop loss
        if stop_loss < 0 and cumulative_profit <= stop_loss:
            print(f"STOP LOSS HIT: ${cumulative_profit:.2f} (Limit: ${stop_loss:.2f})")
        ...
        if take_profit > 0 and cumulative_profit >= take_profit:
            print(f"TAKE PROFIT HIT: ${cumulative_profit:.2f} (Target: ${take_profit:.2f})")
        ...
        # INVERTED LOGIC: Flip the signal
        # If signal is BUY, we SELL. If signal is SELL, we BUY.
        original_signal = signal
        if signal == "BUY":
            direction = "SELL"  # Inverted: Sell on buy signal
        else:
            direction = "BUY"   # Inverted: Buy on sell signal
        
        # Start new trade
        trade_in_progress = True
        
        print(f"Signal: {original_signal} → INVERTED to {direction} at {price:.3f}")
        ...
        trade_history.append({
            ...
            "signal": original_signal,  # Store original signal
            "direction": direction,      # Store actual trade direction (inverted)
            "stake": stake_amount,
...
async def home():
    ...
    for t in reversed(trade_history):
        ...
        # Show both original signal and inverted direction
        display_dir = f"{t['signal']}→{t['direction']}"
        rows += f"<tr style='color:{color}'><td>{t['trade_number']}</td><td>{t['timestamp']}</td><td>{display_dir}</td><td>${t['stake']}</td><td>{t['entry_price']}</td><td>{t['result']}</td><td>${t['profit']}</td></tr>"

    status_color = "green" if auto_trader_running else "red"
    status_text = "RUNNING" if auto_trader_running else "STOPPED"
    if stop_reason:
        status_text += f" ({stop_reason.replace('_', ' ').upper()})"

    profit_color = '#00b894' if cumulative_profit >= 0 else '#d63031'
...
.stats{{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:15px;max-width:800px;margin:20px auto}}
...
.inverted-badge{{background:#e17055;color:white;padding:4px 8px;border-radius:12px;font-size:12px;display:inline-block;margin-left:10px}}
</style>
</head>
<body>
<h1>🚀 Deriv Trading Bot <span class="inverted-badge">INVERTED LOGIC</span></h1>
...
<div class="stats">
    ...
    <div class="stat-box">
        ...
        <div class="stat-value" id="profit" style="color:{profit_color}">${{round(cumulative_profit,2)}}</div>
    </div>
    <div class="stat-box">
    ...
    <div class="stat-box">
        ...
        <div class="stat-value" id="stop_loss">{f'${stop_loss}' if stop_loss < 0 else 'OFF'}</div>
    </div>
    <div class="stat-box">
        <div class="stat-label">Take Profit</div>
        <div class="stat-value" id="take_profit">{f'${take_profit}' if take_profit > 0 else 'OFF'}</div>
    </div>
</div>
...
<form id="startForm">
    ...
    <input name="stop_loss" type="number" step="0.01" value="" placeholder="Stop Loss ($, negative, 0=off)"><br>
    <input name="take_profit" type="number" step="0.01" value="" placeholder="Take Profit ($, positive, 0=off)"><br>
    <button type="submit" class="start-btn">▶ Start Trading (INVERTED)</button>
</form>

<button class="stop-btn" onclick="stopBot()">⏹ Stop Trading</button>

<h3>Trade History (Signal → Inverted Trade)</h3>
<table id="table">
<tr>
    <th>#</th>
    <th>Time</th>
...
document.getElementById("startForm").onsubmit = async (e)=>{{
    ...
    alert("Trading started with INVERTED logic!")
}}
...
async function update(){{
    ...
    document.getElementById("stop_loss").innerText = d.stop_loss < 0 ? "$" + d.stop_loss : "OFF"
    document.getElementById("take_profit").innerText = d.take_profit > 0 ? "$" + d.take_profit : "OFF"
    
    let rows = "<tr><th>#</th><th>Time</th><th>Signal→Trade</th><th>Stake</th><th>Price</th><th>Result</th><th>P/L</th></tr>"
    for(let t of d.trade_history.slice().reverse()){{
        const color = t.result === "WIN" ? "green" : "red"
        const displayDir = t.signal + "→" + t.direction
        rows += `<tr style="color:${{color}}"><td>${{t.trade_number}}</td><td>${{t.timestamp}}</td><td>${{displayDir}}</td><td>$${{t.stake}}</td><td>${{t.entry_price}}</td><td>${{t.result}}</td><td>$${{t.profit}}</td></tr>`
    }}
...
async def start(
    ...
    stop_loss_input: float = Form(0),
    take_profit_input: float = Form(0)
):
    global auto_trader_running
    global api_token
    global stake_amount
    ...
    max_trades = max(0, max_trades_limit)
    
    # Set stop loss (must be negative or 0)
    stop_loss = round(stop_loss_input, 2) if stop_loss_input < 0 else 0
    # Set take profit (must be positive or 0)
    take_profit = round(take_profit_input, 2) if take_profit_input > 0 else 0
    
    stop_reason = ""  # Reset stop reason
    ...
    return {
        "status": "started", 
        "max_trades": max_trades if max_trades > 0 else "unlimited",
        "stop_loss": stop_loss if stop_loss < 0 else "off",
        "take_profit": take_profit if take_profit > 0 else "off",
        "logic": "INVERTED (Sell on Buy, Buy on Sell)"
    }
