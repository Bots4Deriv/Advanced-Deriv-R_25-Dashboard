...
def analyze():
    ...
    # PRINT LIVE LOGS
    # -----------------------
    print(f"[{time.strftime('%H:%M:%S')}] Price: {p} | Signal: {signal} | Prob: {prob} | Momentum: {momentum} | Volatility: {volatility} | Tick Speed: {tick_speed} | Sweep: {liquidity_sweep}")

    if s != "WAIT":
        signals.append({"signal": s, "price": p})
        if len(signals) > 10:
            signals.pop(0)
